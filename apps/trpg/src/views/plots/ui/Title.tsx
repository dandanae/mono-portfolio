...
const Title = ({ title, writer, line }: { title: string; writer: string; line: string }) => {
  ...
  return (
    <button
      ...
      onClick={() => copy(str)}
      className="focus-ring hover-ring m-auto my-4 flex w-full flex-col items-stretch rounded-lg transition-transform duration-300 active:scale-95"
    >
      <CopyWrapper isCopied={isCopied} className="w-[200px] border border-slate-200 bg-white">
        <span
          aria-hidden
        ...
        <span className="flex px-[30px] pb-[24px] text-left! text-[12px]" style={{ color: primaryColor }}>
          w. {writer}
        </span>

        <span className="flex px-[30px] text-[14px] font-bold text-black">
          <b>KPC</b>
        </span>
        <span className="flex h-[20px] px-[30px] text-[14px] text-black">{kpc}</span>
        <span className="flex h-[20px] px-[30px] text-[14px] font-bold text-black">
          <b>PC</b>
        </span>
        <span className="flex h-[20px] px-[30px] text-[14px] text-black">{pc}</span>
